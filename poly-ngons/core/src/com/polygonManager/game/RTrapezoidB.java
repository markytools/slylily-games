...
public class RTrapezoidB extends Polygon {

	public RTrapezoidB(){

	}

	public RTrapezoidB(float xPos, float yPos, boolean reversed, int position){
		super (xPos, yPos, reversed, position, PolygonName.rTrapB);
	...
	protected void createTrianglesAndSquares(){
		...
		for (int triangleCount = 0; triangleCount < PolygonTriangleNum.rTrapB; triangleCount++){
			switch (position){
			case 1: {
				if (!reversed){
					...
					case 2: {
						PolyTriangle triangle = new PolyTriangle(xPos + 2, yPos + 1, 1, this);
						triangles.add(triangle);
					}; break;
					case 3: {
			...
			case 2: {
				if (!reversed){
					...
					case 1: {
						PolyTriangle triangle = new PolyTriangle(xPos + 1, yPos + 2, 1, this);
						triangles.add(triangle);
					}; break;
					case 2: {
						PolyTriangle triangle = new PolyTriangle(xPos, yPos + 1, 3, this);
		...
		for (int squareCount = 0; squareCount < PolygonSquareNum.rTrapB; squareCount++){
			switch (position){
			case 1: {
				if (!reversed){
					PolySquare square = new PolySquare(xPos + 1, yPos + 1, this);
					squares.add(square);
				}
				else {
					PolySquare square = new PolySquare(xPos + 1, yPos + 1, this);
